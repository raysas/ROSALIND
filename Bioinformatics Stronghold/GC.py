...
def readFASTA(filename):
    ...
    for line in my_file:
        if line.startswith('>'):
            ...
            if id!='':#if code is not empty, meaning you already saved a code, add it to its corresponding dna to 
                data[id]=dna
                dna=''
            id=line.strip()
        else: #if it's a dna segment, adss up to the strand
            dna+=line.strip()
    data[id]=dna #saves the last dna strand at the end of the loop
    print(data)
    return data
...
def main():
    s=input("please provide us with the absolute path of your file (make sure it's powershell friendly and escape \ if needed):")
    data=readFASTA(s.strip("'"))
    gc_values=[]
    for key in data:
